...
async def shutdown_on_500(request: Request, exc: Exception):
    logger.error(f"500 Internal Server Error: {exc}")
    # Send response to the client
    response = JSONResponse(
        status_code=500,
        content={"message": "Internal Server Error, restarting..."},
    )
    # Log the error and terminate the process
    logger.error("Shutting down the server due to 500 Internal Server Error.")
    os._exit(1)  # Terminates the process with a non-zero exit code
    return response
